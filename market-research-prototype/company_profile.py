...
{{
  ...
  "channel": "how the venture reaches customers — 'physical' (in-person only, e.g. retail store, restaurant), 'online' (e-commerce, SaaS, app — no physical location required), or 'hybrid' (both). Default to 'online' if unclear.",
  "pricing_mechanism": "how this venture sets its prices — choose the BEST fit: 'standard' (fixed price per unit, the seller sets it), 'cost_plus' (buy/source at X, sell at X * markup — margin is baked into the multiplier), 'evc' (price justified by competitor baseline + value added over them), 'time_based' (charged per hour/day/session), 'commission' (% of deal or transaction value), 'consignment' (sell others' goods, keep a take-rate), 'dynamic' (price varies by demand/inventory), 'subscription' (recurring flat fee), or 'unknown' if genuinely unclear. For a vintage shop sourcing from estate sales and marking up 500%, use 'cost_plus'."
}}
